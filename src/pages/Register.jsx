  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.session) {
        ...
        setMessage("Conta criada. Verifique seu email para confirmar a conta e depois entre no aplicativo.");
      }
    } catch (err) {
    ...
    >
      ...
      <div className="relative mb-6">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-transparent px-3 text-muted-foreground">ou</span>
        </div>
      </div>

      {error && <div className="mb-4 p-3 rounded-lg bg-destructive/10 text-destructive text-sm">{error}</div>}
      {message && <div className="mb-4 p-3 rounded-lg bg-primary/10 text-foreground text-sm">{message}</div>}
